...
class EnterCodeScreen extends StatefulWidget {
  ...
  const EnterCodeScreen({super.key, required this.email, required this.currentState});
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    EcErrorText(),
                    SizedBox(height: AppSize.instance.height * 0.02),
                    EcFieldAndButton(email: widget.email, currentState: widget.currentState),
                    SizedBox(height: AppSize.instance.height * 0.02),
  ...
  void _verifyOtpListener(BuildContext context, VerifyOtpState state) {
    if (state is VerifyOtpDoneState) {
      showModalBottomSheet(
        ...
        showDragHandle: true,
        builder: (context) {
          return PopScope(canPop: false, child: ProductMapBottomSheet(token: state.token,));
        },
      );
