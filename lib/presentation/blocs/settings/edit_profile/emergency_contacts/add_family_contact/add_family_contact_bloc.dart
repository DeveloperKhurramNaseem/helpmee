...
class AddFamilyContactBloc
  ...
  FutureOr<void> _handleAddNewFamilyContactEvent(
    AddNewFamilyContactEvent event,
    Emitter<AddFamilyContactState> emit,
  ) async {
    try {
      emit(AddFamilyContactLoadingState());
      (bool,String) result;
      if(event.contactType == ContactType.family){
        result = await userProfileRepo.addFamilyContact(
        ContactInfo(
          name: event.name,
        ...
        result = await userProfileRepo.addDoctorContact(
        ContactInfo(
          name: event.name,
          phoneNumber: event.phoneNumber,
          whatsappNumber: event.whatsappNumber,
